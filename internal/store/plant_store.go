...
func (s *PlantStore) UpdateCarrier(unitID string, carrier model.CarrierReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
...
func (s *PlantStore) UpdateHeatex(unitID string, comb model.HeatexReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
...
func (s *PlantStore) UpdateTunnel(unitID string, tunnel model.TunnelReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
