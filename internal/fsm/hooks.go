...
// BeltframeDrivePulse counts beltframe-side effects from FSM after hooks (acceptance).
var BeltframeDrivePulse func()

func RegisterBeltframeDriveHook(chain *HookChain) {
	chain.OnAfter(func(ctx context.Context, from, to model.PlantState, event PlantEvent) error {
		_ = ctx
		_ = from
		_ = to
		_ = event
		if BeltframeDrivePulse != nil {
			BeltframeDrivePulse()
		}
		return nil
	})
