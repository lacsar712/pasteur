...
function renderSnapshot(snap) {
  ...
  document.getElementById("pressure").textContent = snap.tunnel.steam_pressure_psi.toFixed(0);
  document.getElementById("mw").textContent = snap.tunnel.output_mw.toFixed(1);
  document.getElementById("carrier-level").textContent = snap.carrier.level_percent.toFixed(1);
  document.getElementById("burner").textContent = snap.heatex.burner_phase;
  document.getElementById("o2").textContent = snap.heatex.excess_o2_pct.toFixed(2);
  trendData.push(snap.tunnel.steam_pressure_psi);
...
document.getElementById("btn-rinse").addEventListener("click", () => postAction("/rinse/start?holder=hmi"));
document.getElementById("btn-rinse-done").addEventListener("click", () => postAction("/rinse/complete?holder=hmi"));
