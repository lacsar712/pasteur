...
func (s *SteamPath) ComputeFlow(snap model.PlantSnapshot, firing bool) float64 {
	...
	loadFactor := snap.Tunnel.OutputMW / math.Max(snap.Settings.TargetMW, 1)
	pressureFactor := snap.Tunnel.SteamPressurePSI / math.Max(snap.Settings.TargetSteamPSI, 1)
...
func (s *SteamPath) ComputeMW(snap model.PlantSnapshot, firing bool) float64 {
	...
	enthalpy := s.enthalpyFromPressure(snap.Tunnel.SteamPressurePSI)
...
func (s *SteamPath) EnthalpyBalance(snap model.PlantSnapshot) float64 {
	inEnergy := snap.Carrier.FeedwaterTPH * 200
	outEnergy := snap.Carrier.SteamFlowTPH * s.enthalpyFromPressure(snap.Tunnel.SteamPressurePSI)
...
func (s *SteamPath) LoadPercent(snap model.PlantSnapshot) float64 {
	...
	return (snap.Tunnel.OutputMW / snap.Settings.TargetMW) * 100
}
